...
const provideLiquidty = async (
    ...
    const ecoFusion = await ethers.getContractAt("EcoFusion", ecoFusionAddress, wallet);
    const resource = await ethers.getContractAt("Resource", resourceAddress, wallet);
    ...
    const mintTx = await resource.mint(ethers.parseUnits("100", 2), wallet.address, {
        gasLimit: 200000
    });
    console.log("mintTx", mintTx.hash);

    const rv = await ContractId.fromEvmAddress(0, 0, reserves[0]).populateAccountNum(client);
    ...
    const receiptAccountAllowance = await responseAccountAllowance.getReceipt(client);

    console.log("Account Allowance Status:", receiptAccountAllowance.status.toString());

    const addLiquidityTx = await reserve.addLiquidity(ethers.parseUnits("50", 2), {
        gasLimit: 200000
    });
